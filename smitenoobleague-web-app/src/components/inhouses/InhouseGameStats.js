...
//custom components
import InhouseTeamTable from "src/components/inhouses/InhouseTeamTable";
//Auth
...
export default function InhouseGameStats({MatchResult, teamsInMatch}) {
    //states
    ...
    const RenderTeamImage = (t) => {   
        return <Image layout="fixed" height={20} width={20} alt={t ? "Order side" : "Chaos side"} title={t ? "Order side" : "Chaos side"} src={t ? "/images/order.png" : "/images/chaos.png"} className="" draggable={false}></Image>;
      };

      const RenderTeamImageLayoutFill = (t) => {
        return <Image height={20} width={20} alt={t?.teamName} title={t?.teamName} src={t ? "/images/order.png" : "/images/chaos.png"} className="" draggable={false}></Image>;
      };
      ...
      const BanWithTeamImage = (banNumber, phase) => {
        return (<>
            <div className="position-relative mt-3 mr-lg-2 mr-1">               
                ...
                <div className="SmallTeamImg position-absolute">{RenderTeamImageLayoutFill(phase == 2 ? (banNumber%2 ? false : true) : (banNumber%2 ? true : false))}</div>
            </div>  
            </>);
      };
        ...
        <>
            ...
            <Row>
                ...
                <Col xl={8} lg={9} md={8} sm={8} xs={12}>
                    {/* Team 1 */}
                    <InhouseTeamTable playerdata={MatchResult?.winners[0]?.firstBanSide ? MatchResult?.winners : MatchResult?.losers } team={true} />
                    {/* Divider */}
                    <Row><Col className="my-auto text-center"><h5 className="mb-0 font-weight-bolder">VS</h5></Col></Row>
                    {/* Team 2 */}
                    <InhouseTeamTable playerdata={MatchResult?.losers[0]?.firstBanSide == false ? MatchResult?.losers : MatchResult?.winners} team={false} />
                </Col>
                <Col xl={2} lg={3} md={4} className="my-auto pl-0">
                    <Alert variant="secondary" className="p-lg-2 p-md-2">
                        ...
                        <hr />
                        <h6 className="d-flex align-items-center justify-content-left"><b><Img webp width={30} height={30} alt={"Phoenixes"} title="Phoenixes" src={require("public/images/Phoenixes.png")} className="" draggable={false}></Img> taken:</b> <span className="mr-1 ml-1">{MatchResult?.winners[0]?.firstBanSide ? totals?.phoenixesWinner : totals?.phoenixesLoser}</span> {RenderTeamImage(true)} <span className="ml-1 mr-1">{MatchResult?.losers[0]?.firstBanSide == false ? totals?.phoenixesLoser : totals?.phoenixesWinner}</span> {RenderTeamImage(false)}</h6>
                        <h6 className="d-flex align-items-center justify-content-left"><b><Img webp width={30} height={30} alt={"Towers"} title="Towers" src={require("public/images/Towers.png")} className="" draggable={false}></Img> taken:</b> <span className="mr-1 ml-1">{MatchResult?.winners[0]?.firstBanSide ? totals?.towersWinner : totals?.towersLoser}</span> {RenderTeamImage(true)} <span className="ml-1 mr-1">{MatchResult?.losers[0]?.firstBanSide == false ? totals?.towersLoser : totals?.towersWinner}</span> {RenderTeamImage(false)}</h6>
                        <h6 className="d-flex align-items-center justify-content-left"><b><Img webp width={30} height={30} alt={"Firegiants"} title="Firegiants" src={require("public/images/Firegiant2.png")} className="rounded"  draggable={false}></Img> taken:</b> <span className="mr-1 ml-1">{MatchResult?.winners[0]?.firstBanSide ? totals?.fireGiantsWinner : totals?.fireGiantsLoser}</span> {RenderTeamImage(true)} <span className="ml-1 mr-1">{MatchResult?.losers[0]?.firstBanSide == false ? totals?.fireGiantsLoser : totals?.fireGiantsWinner}</span> {RenderTeamImage(false)}</h6>
                        <h6 className="d-flex align-items-center justify-content-left"><b><Img webp width={30} height={30} alt={"Goldfuries"} title="Goldfuries" src={require("public/images/Goldfury2.png")} className="rounded" draggable={false}></Img> taken:</b> <span className="mr-1 ml-1">{MatchResult?.winners[0]?.firstBanSide ? totals?.goldFuriesWinner : totals?.goldFuriesLoser}</span> {RenderTeamImage(true)} <span className="ml-1 mr-1">{MatchResult?.losers[0]?.firstBanSide == false ? totals?.goldFuriesLoser : totals?.goldFuriesWinner}</span> {RenderTeamImage(false)}</h6>
                    </Alert>
                </Col>
            </Row>
        </>
