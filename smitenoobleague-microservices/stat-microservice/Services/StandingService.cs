        ...
        public async Task<ActionResult<ScheduleStandingList>> GetStandingsByScheduleId(int? scheduleID)
        {
            try
            {
                ...
                if (foundStandings?.Count() > 0)
                {
                    ...
                    {
                        var last5MatchupIds = await _db.TableMatchResults.Where(tmr => tmr.AwayTeamId == standing.TeamId || tmr.HomeTeamId == standing.TeamId).GroupBy(t => t.ScheduleMatchUpId, (x, y) => new {ScheduleMatchUpId = x, DatePlayed = y.Select(z => z.DatePlayed).Max()}).OrderByDescending(x => x.DatePlayed).Take(5).Select(x => x.ScheduleMatchUpId).ToListAsync();
